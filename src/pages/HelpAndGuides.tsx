...
const MODULE_GUIDES: ModuleGuide[] = [
  ...
  },
  {
    ...
    sections: [
      ...
      { title: 'Dual profit columns', content: 'The orders table shows two profit columns: "Acct $" (accounting profit using actual cost_price) and "Mgmt $" (management profit using original_cost_price + management_labor_cost). This separates GAAP-compliant reporting from performance analysis.' },
...
const SYSTEM_SECTIONS = [
  ...
  { title: 'Management Labor & Dual Profit', icon: <DollarSign className="h-4 w-4 text-primary" />, content: 'Each device can have optional management_labor_cost and management_labor_hours fields. These are used for management profit calculations (Mgmt $) in Orders and the Management View P&L. They do NOT affect accounting books — accounting profit uses actual cost_price. The P&L toggle switches between Accounting View (labor in OpEx) and Management View (labor in COGS, payroll excluded from OpEx).' },
  { title: 'PO Item Type Routing', icon: <ClipboardCheck className="h-4 w-4 text-primary" />, content: 'Each PO line item has an item_type (inventory, repair_parts, or expense). On receive: inventory items go to products table, repair parts go to repair_parts table, expense items create expense records. This allows a single PO to mix product orders with tool/supply purchases from the same supplier.' },
...
const ENTITY_RELATIONSHIPS = [
  ...
  { source: 'purchase_order_items', link: 'purchase_order_id', target: 'purchase_orders', description: 'Line items on a PO (with item_type: inventory/repair_parts/expense)' },
  { source: 'purchase_order_items', link: 'item_type', target: '(routing)', description: 'Determines receive destination: products, repair_parts, or expenses' },
