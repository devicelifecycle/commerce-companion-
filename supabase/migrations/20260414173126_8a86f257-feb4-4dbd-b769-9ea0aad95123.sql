...
SELECT id, '4400', 'Direct / Invoice Sales - VES', 'revenue', 'other_revenue', 'credit', true, 'Revenue from direct invoices and off-marketplace sales for VES'
FROM public.companies WHERE code = 'VES'
ON CONFLICT (company_id, account_code) DO NOTHING;

INSERT INTO public.chart_of_accounts (company_id, account_code, account_name, account_type, account_subtype, normal_balance, is_system_account, description)
SELECT id, '4401', 'Direct / Invoice Sales - TGW', 'revenue', 'other_revenue', 'credit', true, 'Revenue from direct invoices and off-marketplace sales for TGW'
FROM public.companies WHERE code = 'TGW'
ON CONFLICT (company_id, account_code) DO NOTHING;
